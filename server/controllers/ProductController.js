...
export default class ProductController {
  ...
  static searchForProduct(req, res) {
    const { query_string, all_words='off', page=0, limit=20, description_length=200 } = req.query;
    if (!query_string) {
    ...
    .then(count => {
      Model.sequelize.query(`CALL catalog_search('${query_string}', '${all_words}', ${description_length}, ${limit}, ${page})`)
        .then(products => {
          return res.status(206).json({
            "count": count[0]['count(*)'],
            "rows": products
